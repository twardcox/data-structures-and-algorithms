...
const findHappiness = arr => {
  let pattern = /\:\)/;
  let newArr = [];
  arr.forEach(v => (pattern.test(v) ? newArr.push(v) : ''));
  return newArr;
};
...
const standardizePhoneNumbers = arr => {
  return arr.map(v => v.match(/[^\(\)\-\s]/g).join(''));
};
...
const onlyOddChars = str => {
  let newArr = [];
  str.split('').forEach((v, i) => (i % 2 ? newArr.push(v) : ''));
  return newArr.join('');
};
/* ------------------------------------------------------------------------------------------------
CHALLENGE 5
...
const allHappy = arr => {
  let pattern = /\:\)/;
  let exists = true;
  arr.forEach(v => (pattern.test(v) ? '' : (exists = false)));
  return exists;
};
...
const findAnything = (arr, target) => {
  ...
  arr.forEach(v => (v.includes(target) ? newArr.push(v) : ''));
  return newArr;
};
...
const findEvery = (arr, target) => {
  ...
  arr.forEach(v => (v.includes(target) ? '' : (newArr = false)));
  return newArr;
};
...
const unenrollBrook = arr => {
  return arr.map(v => v.filter(w => w.match(/^((?!Brook).)*$/gm)));
};
...
xdescribe('Testing challenge 8', () => {
  test('It should remove Brook from all courses', () => {
    const roster = [['Michelle', 'Allie', 'Brook TESTING'], ['Brook Riggio', 'hey look it\'s Brook', 'Jennifer'], ['Nicholas', 'Sam', 'Scott', 'Vinicio']];

    expect(unenrollBrook(roster)).toStrictEqual([['Michelle', 'Allie'], ['Jennifer'], ['Nicholas', 'Sam', 'Scott', 'Vinicio']]);
