...
const filterStringsWithVowels = arr => {
  return arr.filter(v => (/[AaEeIiOoUu]/g.test(v) ? v : ''));
};
...
const getBaseStatGreaterThan = (arr, minBaseStat) => {
  return arr.filter((v, i, a) => v.baseStat > minBaseStat);
};
/* ------------------------------------------------------------------------------------------------
CHALLENGE 5
...
const getStatName = (arr, minBaseStat) => {
  let newArr = arr.filter(v => v.baseStat > minBaseStat);
  return newArr.map(v => (v ? v.stat.name : ''));
};
...
const getCharactersWithoutChildren = arr => {
  return arr.filter((v, i, a) => !v.children);
};
...
xdescribe('Testing challenge 5', () => {
  ...
  test('It should work for non-snorlax data', () => {
    expect(getStatName([{ baseStat: 10, stat: { name: 'one' } }, { baseStat: -85, stat: { name: 'two' } }, { baseStat: 0, stat: { name: 'three' } }, { baseStat: -50, stat: { name: 'four' } }], -60)).toStrictEqual(['one', 'three', 'four']);
  });
});
