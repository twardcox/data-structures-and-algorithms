...
const citiesAtoJ = arr => {
  let patt = new RegExp(/\b[A-J]\w*/g);
  return arr.filter(v => patt.test(v));
};
...
let hangman = str => {
  return str.replace(/[aAeEiIoOuU]/gi, '_');
};
...
const findShells = str => {
  let patt = new RegExp(/\w*ells\b/g);
  if (str.match(patt) === null) {
    return [];
  } else {
    return str.match(patt);
...
describe('Testing challenge 3', () => {
  ...
  test('It should return the cities whose names begin with the letters A through J', () => {
    ...
    expect(citiesAtoJ(['Albuquerque', 'Philadelphia', 'Chicago', 'Newark', 'Sacramento', 'Eugene'])).toEqual(expect.arrayContaining(['Albuquerque', 'Chicago', 'Eugene']));
  });
...
describe('Testing challenge 5', () => {
  ...
  test('It should only return words that are immediately followed by a space', () => {
    ...
    expect(noPunctuation('Given by our hand in the meadow that is called Runnymede, between Windsor and Staines, on the fifteenth day of June in the seventeenth year of our reign (i.e. 1215: the new regnal year began on 28 May).')).toEqual(
      expect.arrayContaining(['Given ', 'by ', 'our ', 'hand ', 'in ', 'the ', 'meadow ', 'that ', 'is ', 'called ', 'between ', 'Windsor ', 'and ', 'on ', 'the ', 'fifteenth ', 'day ', 'of ', 'June ', 'in ', 'the ', 'seventeenth ', 'year ', 'of ', 'our ', 'reign ', 'the ', 'new ', 'regnal ', 'year ', 'began ', 'on ', '28 '])
    );
  });
