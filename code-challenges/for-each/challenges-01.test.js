...
const createList = availableItems => {
  let groceryList = [];
  availableItems.forEach(v => (v['available'] ? groceryList.push(v['name']) : ''));
  return groceryList;
};
...
const fizzbuzz = arr => {
  let output = [];
  arr.forEach(v => {
    v % 3 === 0 ? (v % 5 === 0 ? output.push('Fizz Buzz') : output.push('Fizz')) : v % 5 === 0 ? output.push('Buzz') : output.push(v);
  });
  return output;
};
