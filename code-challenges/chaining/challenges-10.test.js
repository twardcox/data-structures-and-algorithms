...
const totalSum = input => {
  return input.reduce((t, v) => (t += v.reduce((a, n) => (a += n))), 0);
};
...
const divisibleByFiveTwoToThePower = input => {
  let filteredArr = input.map(v => v.filter(n => Number.isInteger(n)));
  let divByFive = filteredArr.map(v => v.filter(n => (n % 5 ? 0 : 1)));
  let raiseTwoBy = divByFive.map(v => v.map(n => 2 ** n));
...
let findMaleAndFemale = data => {
  let filterData = data.filter(v => v.gender === 'male' || v.gender === 'female');
  let nameData = filterData.map(v => v.name);
  let printString = '';
  nameData.forEach((v, i) => {
    i < nameData.length - 1 ? (printString += v + ' and ') : (printString += v);
  });
  return printString;
};
