...
const gruffaloCrumble = {
  ...
  ingredients: ['1 medium-sized Gruffalo', '8 pounds oats', '2 pounds brown sugar', '4 pounds flour', '2 gallons pure maple syrup', '16 cups chopped nuts', '1 pound baking soda', '1 pound baking powder', '1 pound cinnamon', '6 gallons melted butter', '2 gallons fresh water'],
  steps: ['Pre-heat a large oven to 375', 'De-prickle the gruffalo', 'Sprinkle with cinnamon, sugar, flour, and nuts', 'Mix until evenly distributed', 'Grease a 3-foot x 3-foot casserole dish', 'Combine gruffalo compote with water to maintain moisture in the oven', 'Fold together remaining ingredients to make the crisp', 'Spread the crisp evenly over the gruffalo mixture', 'Bake for 12-15 hours']
};
...
const listFoods = recipe => {
  ...
  for (let i = 0; i < ingredients.length; i++) {
    let spliced = ingredients[i].split(' ');

    result.push(spliced.slice(2).join(' '));
  }
...
const stepActions = recipe => {
  ...
  for (let i = 0; i < ingredients.length; i++) {
    ...
    result.push(spliced.slice(0, 1).join(' '));
  }
...
const removeEvenValues = arr => {
  arr.forEach((v, i, a) => (!(v % 2) ? a.splice(i, 1) : false));
};
/* ------------------------------------------------------------------------------------------------
CHALLENGE 7
...
const totalSumCSV = str => {
  return Number(str.split(',').reduce((t, n) => Number(t) + Number(n)));
};
