...
const toTitleCase = arr => {
  ...
  arr.forEach(v => {
    v.replace(/\w\S*/g, txt => {
      txt.charAt(0) === txt.charAt(0).toUpperCase() ? newArr.push(txt) : newArr.push(txt.charAt(0).toUpperCase() + txt.substr(1).toLowerCase());
    });
  });
...
describe('Testing challenge 3', () => {
  ...
  test('It should sort items by name', () => {
    expect(sortBy('name', [{ name: 'Sweatshirt', price: 45 }, { name: 'Bookmark', price: 2.5 }, { name: 'Tote bag', price: 15 }])).toStrictEqual([{ name: 'Bookmark', price: 2.5 }, { name: 'Sweatshirt', price: 45 }, { name: 'Tote bag', price: 15 }]);
  });
});
