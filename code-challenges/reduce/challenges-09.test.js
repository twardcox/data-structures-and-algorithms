...
const reversedString = str => {
  return str.split('').reduce((t, v) => (v += t));
};
...
const countNumberOfChildren = arr => {
  return arr.reduce((t, v) => {
    t += v.children ? v.children.length : 0;
    return t;
  }, 0);
};
...
const calculateAverage = arr => {
  return arr.reduce((t, v) => t + v) / arr.length;
};
